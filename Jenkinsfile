pipeline {
  environment {
    ...
    IC_WEBAPP_PORT = "80"
    PGADMIN_PORT = """${sh(returnStdout: true, script: 'grep PGADMIN_URL releases.txt | awk -F": " "{print \$2}" | grep -Po "//.+?:.+/?" | grep -Po "(?<=:)(\\d+)" || echo "80"')}"""
    ...
    stage ('Test docker image') {
        steps{
            ...
            sh '''
                ...
                docker network connect test jenkins-build-ci
                docker network connect test ${CONTAINER_NAME}
                sleep 2;
                curl http://${CONTAINER_NAME}:8080 | grep -q "IC GROUP";
                docker network disconnect test jenkins-build-ci
